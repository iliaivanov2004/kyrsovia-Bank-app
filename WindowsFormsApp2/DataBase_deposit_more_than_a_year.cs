...
{
    public partial class DataBase_deposit_more_than_a_year : Form
    {
        public DataBase_deposit_more_than_a_year()
        {
            InitializeComponent();
        ...
        private void LoadData() // вывод базы данных в DataGridView 
        {
            ...
            myConnection.Open();
            string query = "SELECT * FROM deposit_more_than_a_year ORDER BY percent";
        ...
        {
            MySqlConnection myConnection = new MySqlConnection("server = localhost; port = 3306;username = root; password = root;database = banks");
            MySqlCommand command_delete = new MySqlCommand("DELETE FROM `deposit_more_than_a_year` WHERE `Name` = @Name", myConnection);
        ...
        {
            MySqlConnection myConnection = new MySqlConnection("server = localhost; port = 3306;username = root; password = root;database = banks");
            MySqlCommand command = new MySqlCommand("INSERT INTO `deposit_more_than_a_year` (`Name`, `Status`, `Percent`) VALUES (@Name, @Status, @Percent)", myConnection);
