...
{
    public partial class DataBase_deposit_181_year : Form
    {
        public DataBase_deposit_181_year()
        {
            InitializeComponent();
        ...
        private void LoadData() // вывод базы данных в DataGridView 
        {
            ...
            myConnection.Open();
            string query = "SELECT * FROM deposit_181_year ORDER BY percent";
        ...
        {
            MySqlConnection myConnection = new MySqlConnection("server = localhost; port = 3306;username = root; password = root;database = banks");
            MySqlCommand command_delete = new MySqlCommand("DELETE FROM `deposit_181_year` WHERE `Name` = @Name", myConnection);
        ...
        private void NameFieldDelete_TextChanged(object sender, EventArgs e) //проверка введен ли текст в поле NameFieldDelete
        {
            DeleteButton_CheckTextBoxes();
        }

        private void NameField_TextChanged(object sender, EventArgs e) //проверка введен ли текст в поле NameField
        {
            CheckTextBoxes();
        }

        private void StatusField_TextChanged(object sender, EventArgs e) //проверка введен ли текст в поле StatusField
        {
            CheckTextBoxes();
        }

        private void PercentField_TextChanged(object sender, EventArgs e) //проверка введен ли текст в поле  PercentField
        {
            CheckTextBoxes();
        }
        ...
        {
            MySqlConnection myConnection = new MySqlConnection("server = localhost; port = 3306;username = root; password = root;database = banks");
            MySqlCommand command = new MySqlCommand("INSERT INTO `deposit_181_year` (`Name`, `Status`, `Percent`) VALUES (@Name, @Status, @Percent)", myConnection);
